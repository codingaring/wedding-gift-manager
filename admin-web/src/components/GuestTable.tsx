...
export default function GuestTable({ onEdit }: GuestTableProps) {
  ...
  const columns = useMemo(() => [
    ...
    col.accessor('amount', {
      header: '금액',
      cell: (info) => <span className="font-semibold">{fmt(info.getValue())}원</span>,
    }),
      ...
      cell: (info) => (
        <div className="flex gap-1">
          ...
          <button
            onClick={(e) => {
              e.stopPropagation();
              if (window.confirm(`"${info.row.original.name}" 항목을 삭제하시겠습니까?`)) {
      ...
      {table.getPageCount() > 1 && (
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200">
          <span className="text-sm text-gray-500">
            {guests.length}건 중 {table.getState().pagination.pageIndex * table.getState().pagination.pageSize + 1}
            -{Math.min((table.getState().pagination.pageIndex + 1) * table.getState().pagination.pageSize, guests.length)}
          </span>
